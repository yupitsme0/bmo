...
package Bugzilla::Extension::ProductBrowser::Queries;

use strict;

use base qw(Exporter);
@Bugzilla::Extension::ProductBrowser::Queries::EXPORT = qw(
    total_bugs
    total_open_bugs
    total_closed_bugs
    by_version
...
use Bugzilla::Extension::ProductBrowser::Util qw(open_states closed_states);
...
sub recently_opened {
    ...
    if ($date_from && $date_to) {
        ...
        $date_part = "AND bugs.creation_ts >= NOW() - " . $dbh->sql_interval('?', 'DAY');
        push(@values, $days);
    }
